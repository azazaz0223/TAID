...
@section('container')
    ...
    <nav class="" aria-label="breadcrumb">
        <ol class="breadcrumb d-flex justify-content-start lh-lg m-0 ms-3">
            ...
            <li class="breadcrumb-item col">課程資訊</li>
        </ol>
    </nav>
    <div class="container-fluid p-3 m-1">

        <div class="card col-12 rounded-3 bg-white mb-4">
            <h2 class="fs-5 p-3 fw-bold border-bottom">課程資訊編輯</h2>
            ...
            <form id="update">
                <div class="card-body border-bottom">
                    ...
                    <div class="d-flex justify-content-start gap-3 mb-3">
                        ...
                        <div class="col">
                            <textarea name="subtitle" class="form-control search_input easein mb-0" rows="2" placeholder="請輸入課程副標">{{ $course->subtitle }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-start gap-3 mb-3">
                        ...
                        <div class="col">
                            <select class="select form-control" name="status">
                                ...
                                <option value="1" @selected($course->status == 1)>上架</option>
                                <option value="0" @selected($course->status == 0)>下架</option>
                            </select>
                        </div>
                ...
                <div class="card-body border-bottom d-flex justify-content-between gap-3">
                    ...
                    <div class="col-8 card-body fs-6 gray_l rounded-3">
                        <label class="mb-2">預覽課程圖片</label>
                        <div class="p-0">
                            <img id="newsImg" class="mt-3" src="{{ asset($course->image) }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body border-bottom">
                    <div class="d-flex justify-content-start gap-3 mb-3">
                        ...
                        <div class="col">
                            <textarea name="content_text" class="form-control search_input easein mb-0" rows="2" placeholder="請輸入內文文字">{{ $course->content_text }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body border-bottom d-flex justify-content-between gap-3">
                    ...
                    <div class="col-8 card-body fs-6 gray_l rounded-3">
                        ...
                        <div class="p-0">
                            <img id="newsContentImg" class="mt-3" src="{{ asset($course->content_image) }}">
                        </div>
                    </div>
                </div>
            </form>
            <div class="card-body">
                ...
                <button type="button" onclick="location.href = '{{ route('backend.course.index') }}';"
                    class="btn btn-secondary border-0 rounded-3 float-end shadow-sm px-3 me-2">返回列表</button>
                <button type="button" onclick="updateBtn({{ $course->id }})"
    ...
    <script>
        ...
        function updateBtn(id) {
            url = "{{ route('backend.course.update', ':id') }}";
                    ...
                    if (response.code == '00') {
                        if (response.code == '00') {
                            Swal.fire({
                                ...
                                location.href = "{{ route('backend.course.index') }}";
                            });
                        };
                    };
                },
